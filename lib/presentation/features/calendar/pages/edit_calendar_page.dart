...
class _EditCalendarPageState extends State<EditCalendarPage> {
  ...
  void _updateCalendar() {
    if (_formKey.currentState!.validate()) {
      // Tạo đối tượng CalendarEntity đã được cập nhật
      final updatedCalendar = CalendarEntity(
        id: widget.calendar.id,
        name: _nameController.text.trim(),
        description: _descriptionController.text.trim(),
        isDefault: widget.calendar.isDefault,
      );
      // Gửi event mới SaveCalendarSubmitted
      context.read<CalendarBloc>().add(
        SaveCalendarSubmitted(calendar: updatedCalendar),
      );
    }
  }
